...
class TestDatasetResource(ReadOnlyResourceTestCaseMixin, TestCase):
	...
	def test_get_list_metadata(self):
		...
		msg = 'When no filter is requested, the metadata field uri must be exactly the metadata ressource list url'
		response = self.api_client.get(self.get_resource_uri(), format='json', authentication=self.test_user_authentication)
		self.assertGetListResponseContains(response, metadata = [{'uri': metadata_resource_list_url, 'estimated_count': 1}, None], msg=msg)
		
		msg = 'When a simple filter for the metadata resource is requested, the metadata field uri must include the filter'
		data = {'oid': self.test_metadata.oid}
		response = self.api_client.get(self.get_resource_uri(), data=data, format='json',authentication=self.test_user_authentication)
		self.assertGetListResponseContains(response, metadata = [{'uri': metadata_resource_list_url + '?' + urlencode(data), 'estimated_count': 1}, None], msg=msg)
		
		msg = 'When multiple filters for the metadata resource are requested, the metadata field uri must include the filters'
	...
	def test_get_detail_metadata(self):
		...
		response = self.api_client.get(self.get_resource_uri(self.test_dataset1), format='json', authentication=self.test_user_authentication)
		self.assertGetDetailResponseContains(response, metadata = {'uri': metadata_resource_list_url, 'estimated_count': 1}, msg=msg)
