...
class TestDatasetResource(ReadOnlyResourceTestCaseMixin, TestCase):
	...
	def setUp(self):
		...
		self.test_dataset2 = create_test_dataset(
			name='test dataset 2',
			metadata_model=None,
			characteristic_names=[self.test_characteristic1.name, self.test_characteristic2.name],
		)
	...
	def test_get_list_metadata(self):
		...
		self.assertGetListResponseContains(
			response, metadata=[{'resource_uri': metadata_resource_list_url, 'count': 1}, None], msg=msg
		)
		...
		self.assertGetListResponseContains(
			response, metadata=[{'resource_uri': metadata_resource_list_url + '?' + urlencode(data), 'count': 1}, None], msg=msg
		)
		...
		self.assertGetListResponseContains(
			response,
			metadata=[
				{'resource_uri': metadata_resource_list_url + '?' + urlencode({'oid': self.test_metadata.oid}), 'count': 1},
				None,
			],
			msg=msg,
		)
	...
	def test_get_detail_metadata(self):
		...
		self.assertGetDetailResponseContains(
			response, metadata={'resource_uri': metadata_resource_list_url, 'count': 1}, msg=msg
		)

		msg = 'When the dataset metadata_content_type is None, the metadata field must be None'
		response = self.api_client.get(
			self.get_resource_uri(self.test_dataset2), format='json', authentication=self.test_user_authentication
		)
