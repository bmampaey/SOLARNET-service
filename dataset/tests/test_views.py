...
class TestDataView(TestCase):
	...
	def setUp(self):
		...
		self.test_data_location = self.test_dataset.data_locations.create(
			file_url='/static/test_file1.fits', file_size=256, thumbnail_url='/static/test_thumbnail.png'
		)
		self.test_metadata = self.test_dataset.metadata_model.objects.create(
			oid='test_metadata', data_location=self.test_data_location
		)
...
class TestThumbnailView(TestCase):
	...
	def test_get(self):
		...
		msg = 'A GET for a valid dataset and metadata oid must return a redirect response'
		url = reverse(
			'dataset:thumbnail', kwargs={'dataset_name': self.test_dataset.name, 'metadata_oid': self.test_metadata.oid}
		)
		response = self.client.get(url)
