...
class TestDataLocationAdmin(TestAdminMixin, TestCase):
	...
	def setUp(self):
		...
		self.test_dataset_datalocation = self.test_dataset.data_locations.create(
			file_url='https://test.dataset.com/file.fits', file_size=1024, file_path='test_path/file.fits', offline=False
		)

		# Create a data location for the other dataset
		self.other_dataset_datalocation = self.other_dataset.data_locations.create(
			file_url='https://other.dataset.com/file.fits', file_size=1024, file_path='test_path/file.fits', offline=False
		)
	...
	def test_list_url(self):
		...
		self.assertInHtmlResponse(
			response,
			self.get_change_url(self.test_dataset_datalocation),
			self.get_change_url(self.other_dataset_datalocation),
			msg=msg,
		)
	...
	def test_add_url(self):
		...
		self.assertFormError(
			response, 'adminform', 'dataset', ModelChoiceField.default_error_messages['invalid_choice'], msg_prefix=msg
		)
	...
	def test_change_url(self):
		...
		self.assertObjectUpdated(
			self.test_dataset_datalocation, 'file_url', 'file_size', 'file_path', 'thumbnail_url', 'offline', msg=msg
		)
		...
		self.assertRedirects(response, self.get_list_url(DataLocation), msg_prefix=msg)
		self.assertObjectUpdated(
			self.test_dataset_datalocation, 'dataset', 'file_url', 'file_size', 'file_path', 'thumbnail_url', 'offline', msg=msg
		)
