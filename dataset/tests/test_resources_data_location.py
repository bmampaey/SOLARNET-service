...
class TestDataLocationResource(ResourceTestCaseMixin, TestCase):
	...
	def setUp(self):
		...
		self.test_data_location = self.test_dataset.data_locations.create(
			file_url='https://test.com/file.fits', file_size=1024, file_path='test_path/file.fits', offline=True
		)
	...
	def test_post_list(self):
		...
		self.assertObjectCreated(
			self.test_dataset.data_locations, file_url=data['file_url'], file_size=data['file_size'], msg=msg
		)
	...
	def test_patch_detail(self):
		...
		response = self.api_client.patch(
			self.get_resource_uri(self.test_data_location),
			data=data,
			format='json',
