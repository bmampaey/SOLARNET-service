...
class DatasetResource(ModelResource):
	...
	metadata = fields.DictField(
		readonly=True,
		help_text='The URI and the estimated count for the metadata of the dataset with current filters applied',
	)
	...
	def dehydrate_metadata(self, bundle):
		...
		if not query_dict:
			with connection.cursor() as cursor:
				cursor.execute(
					'SELECT reltuples::bigint AS estimate FROM pg_class where relname = %s;', (metadata_model._meta.db_table,)
				)
		...
		return {'resource_uri': resource_uri, 'count': metadata_count}
