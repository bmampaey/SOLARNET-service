...
from dataset.models import KEYWORD_TYPE_CONVERTERS, Dataset, KeywordType
...
class {{ model_name }}Admin(BaseMetadataAdmin):
	readonly_fields = [{% for keyword in constant_value_keywords %}'{{ keyword.name }}',{% endfor %}]
{% endautoescape %}"""
...
class Command(BaseCommand):
	...
	def handle(self, **options):
		...
		try:
			with open(file_path, file_mode, encoding='UTF_8') as file:
				file.write(self.get_admin_file(dataset, model_name, constant_value_keywords))
		except Exception as why:
			raise CommandError('Could not write file %s: %s' % (file_path, why)) from why
		else:
			self.log.info('Wrote file %s', file_path)
	...
	def get_keywords(self, dataset):
		...
		for keyword in keywords:
			if keyword.constant_value is not None:
				...
				try:
					keyword.constant_value_representation = repr(keyword.converted_constant_value)
	...
	def get_admin_file(self, dataset, model_name, constant_value_keywords):
