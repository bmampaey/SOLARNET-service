...
class BaseMetadataResource(ModelResource):
	...
	def base_urls(self):
		...
		# Override the URLs to group ressources under a common metadata/ prefix
		return [
			re_path(r'^metadata/(?P<resource_name>%s)/$' % self._meta.resource_name, self.wrap_view('dispatch_list'), name='api_dispatch_list'),
			re_path(r'^metadata/(?P<resource_name>%s)/schema/$' % self._meta.resource_name, self.wrap_view('get_schema'), name='api_get_schema'),
			re_path(r'^metadata/(?P<resource_name>%s)/set/(?P<%s_list>.*?)/$' % (self._meta.resource_name, self._meta.detail_uri_name), self.wrap_view('get_multiple'), name='api_get_multiple'),
			re_path(r'^metadata/(?P<resource_name>%s)/(?P<%s>.*?)/$' % (self._meta.resource_name, self._meta.detail_uri_name), self.wrap_view('dispatch_detail'), name='api_dispatch_detail'),
		]
