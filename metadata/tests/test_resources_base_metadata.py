...
class TestBaseMetadataResource(ResourceTestCaseMixin, TestCase):
	...
	def setUp(self):
		...
		self.test_data_location1 = self.test_dataset.data_locations.create(
			file_url='https://test.com/file1.fits', file_size=1, file_path='test_path/file1.fits', offline=False
		)
		self.test_data_location2 = self.test_dataset.data_locations.create(
			file_url='https://test.com/file2.fits', file_size=2, file_path='test_path/file2.fits', offline=True
		)
	...
	def test_get_list_filtered_search(self):
		...
		msg = (
			'When a complex search expression is requested for a non-existing keyword, a bad request response must be returned'
		)
	...
	def test_post_list(self):
		...
		self.assertObjectCreated(
			self.test_dataset.metadata_model.objects.all(),
			oid=data['oid'],
			data_location=self.test_dataset.data_locations.get(file_url=data_location['file_url']),
			msg=msg,
		)
		...
		self.assertQuerysetEqual(
			self.test_dataset.metadata_model.objects.filter(oid=data['oid']).values_list('tags__name', flat=True),
			[self.test_tag3.name, 'new tag'],
			ordered=False,
			msg=msg,
		)
	...
	def test_patch_detail(self):
		...
		response = self.api_client.patch(
			self.get_resource_uri(self.test_metadata1), data=data, format='json', authentication=self.test_user_authentication
		)
		self.assertHttpUnauthorized(response, msg=msg)
		...
		self.test_user.user_permissions.add(
			Permission.objects.get(codename='change_basemetadatatest'), Permission.objects.get(codename='change_tag')
		)
		data = self.get_test_patch_data()
		response = self.api_client.patch(
			self.get_resource_uri(self.test_metadata1), data=data, format='json', authentication=self.test_user_authentication
