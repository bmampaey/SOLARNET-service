...
class UserResource(Resource):
	...
	def get_response_data(self, user):
		...
		return {'first_name': user.first_name, 'last_name': user.last_name, 'api_key': user.api_key.key}
	...
	def base_urls(self):
		...
		return [
			re_path(
				r'^(?P<resource_name>%s)/$' % self._meta.resource_name, self.wrap_view('dispatch_list'), name='api_dispatch_list'
			),
			re_path(
				r'^(?P<resource_name>%s)/schema/$' % self._meta.resource_name, self.wrap_view('get_schema'), name='api_get_schema'
			),
		]
	...
	def post_list(self, request, **kwargs):
		...
		try:
			...
			return self.error_response(
				request, {'error': 'An account with this email already exists'}, response_class=HttpBadRequest
			)
		else:
