...
class TestUserResource(SvoApiTestCaseMixin, TransactionTestCase):
	...
	def test_get_list(self):
		...
		self.assertDictEqual(
			self.deserialize(response),
			{
				...
				'api_key': self.test_user.api_key.key,
			},
			msg=msg,
		)
	...
	def test_post_list(self):
		...
		self.assertDictEqual(
			self.deserialize(response),
			{'first_name': user.first_name, 'last_name': user.last_name, 'api_key': user.api_key.key},
			msg=msg,
		)
	...
	def test_delete_list(self):
		...
		response = self.api_client.delete(
			self.get_resource_uri(), format='json', authentication=self.test_user_authentication
		)
