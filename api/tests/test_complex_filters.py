...
class TestGetComplexFilter(SimpleTestCase):
	...
	def test_error(self):
		...
		msg = (
			'In case of an expression that does not conform, raise a InvalidFilterError exception if ignore_bad_filters is False'
		)
	...
	def test_complex_expression(self):
		...
		self.assertEqual(
			get_complex_filter('name=bob or name=bobette or name=sidonie'),
			Q(name='bob') | Q(name='bobette') | Q(name='sidonie'),
			msg=msg,
		)
		...
		self.assertEqual(
			get_complex_filter('name=bob or age__gte=18 and age__lte=65'),
			Q(name='bob') | Q(age__gte='18') & Q(age__lte='65'),
			msg=msg,
		)
