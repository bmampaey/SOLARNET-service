...
class DataSelectionFilesystemOperations(Operations):
	...
	def get_data_location_values(self, data_selection, *data_location_values, **extra_values):
		...
		try:
			metadata_queryset = get_metadata_queryset(
				data_selection.dataset.metadata_model, data_selection.query_string, data_selection.owner
			)
		except ValueError:
			raise FuseOSError(errno.ENOENT)

		return metadata_queryset.filter(
			data_location__offline=False, data_location__file_size__lte=settings.FTP_MAX_FILE_SIZE
		).values(**values)
	...
	def getattr(self, path, fh=None):
		...
		if data_selection is None:  # path is /
			...
			data_location = (
				self.get_data_location_values(data_selection, 'file_path', 'file_size', 'update_time')
				.filter(Q(file_path__exact=file_path) | Q(file_path__startswith=file_path + self.PATH_SEP))
	...
	def readdir(self, path, fh=None):
		...
		if data_selection is None:  # path is /
			...
			path_part = Func(
				F('file_path'), Value(self.PATH_SEP), Value(parts_count), function='SPLIT_PART', output_field=CharField()
			)
			data_locations = self.get_data_location_values(data_selection, 'file_path', path_part=path_part).filter(
				file_path__startswith=directory_path
			)
			dirents += list(data_locations.values_list('path_part', flat=True).distinct())
	...
	def open(self, path, flags):
		...
		if path not in self.path_urls:
			...
			data_location = (
				self.get_data_location_values(data_selection, 'file_path', 'file_url').filter(file_path=file_path).first()
			)
...
class Command(BaseCommand):
	...
	def add_arguments(self, parser):
		...
		parser.add_argument(
			...
			'-d',
			action='store_true',
			help='Mount the FUSE filesystem with the "-d" option, and list all the data selections under the root path as /uuid',
		)
		parser.add_argument(
			'--foreground',
			'-f',
			action='store_true',
			help='Mount the FUSE filesystem with the "-f" option (handy for debugging)',
		)
		parser.add_argument(
			'--user',
			'-u',
			metavar='USERNAME',
			default='ftp',
			help='The name of the user that will own the files and directories',
		)
		parser.add_argument(
			'--fsname',
			default='svo_data_selection',
			help='The name of the filesystem, for e.g shown when calling the shell command mount',
		)
	...
	def handle(self, **options):
		...
		if options['foreground']:
			...
			console.setFormatter(
				logging.Formatter(
					fmt='%(asctime)s.%(msecs)d [%(levelname)s] Thread %(thread)d Function %(module)s.%(funcName)s: %(message)s',
					datefmt='%H:%M:%S',
				)
			)
