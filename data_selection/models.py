...
class DataSelection(models.Model):
	owner = models.ForeignKey(
		'auth.User', related_name='data_selections', related_query_name='data_selection', on_delete=models.CASCADE
	)
	dataset = models.ForeignKey(
		'dataset.Dataset', on_delete=models.CASCADE, related_name='data_selections', related_query_name='data_selection'
	)
	...
	uuid = models.UUIDField(
		'Unique identifier for web access', blank=True, default=uuid4, editable=False, unique=True, db_index=True
	)
	...
	def zip_download_url(self):
		return '%s%s' % (
			...
			reverse('data_selection:data_selection_download_zip', kwargs={'uuid': self.uuid}),
		)
