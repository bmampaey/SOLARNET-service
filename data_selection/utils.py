...
class DataSelectionZipIterator:
	...
	def get_files(self):
		...
		for file_path, file_url, file_size, offline in self.get_data_locations():
			...
			if offline:
				self.missing_files[file_path] = (
					'File is not accessible through the SVO, please contact the dataset manager to request access'
				)

			elif file_size > settings.ZIP_ARCHIVE_MAX_FILE_SIZE:
				self.missing_files[file_path] = (
					'File is too large to be included in a ZIP archive, you can download it directly at %s or contact the dataset manager'
					% file_url
				)
	...
	def get_data_locations(self):
		...
		return (
			get_metadata_queryset(
				self.data_selection.dataset.metadata_model, self.data_selection.query_string, self.data_selection.owner
			)
			.filter(data_location__isnull=False)
			.values_list(
				'data_location__file_path', 'data_location__file_url', 'data_location__file_size', 'data_location__offline'
			)
			.iterator()
		)
