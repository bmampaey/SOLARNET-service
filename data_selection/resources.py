...
class DataSelectionResource(ModelResource):
	...
	zip_download_url = fields.CharField(
		attribute='zip_download_url', readonly=True, help_text='A URL to download the data selection as a ZIP archive'
	)
	ftp_download_url = fields.CharField(
		attribute='ftp_download_url', readonly=True, help_text='A URL to download the data selection via FTP'
	)
	...
	metadata = fields.DictField(
		readonly=True,
		help_text='The URI and the estimated count for the metadata of the data selection with current filters applied',
	)
	...
	def dehydrate_metadata(self, bundle):
		...
		try:
			resource_uri = reverse(
				'api_dispatch_list',
				kwargs={'resource_name': metadata_resource._meta.resource_name, 'api_name': metadata_resource._meta.api_name},
			)
