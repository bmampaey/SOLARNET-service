...
class TestDataSelectionResource(ResourceTestCaseMixin, TestCase):
	...
	def setUp(self):
		...
		self.other_user = User.objects.create_user(
			username='other_test@test.com', email='other_test@test.com', password='test', first_name='test', last_name='test'
		)
		self.other_user_authentication = self.get_authentication(self.other_user, password='test')
		self.other_test_data_selection = self.other_user.data_selections.create(
			dataset=self.test_dataset, description='other user test data selection'
		)
		...
		self.test_patch_data = {'query_string': 'oid=test_metadata1', 'description': 'other description'}
	...
	def test_post_list(self):
		...
		msg = (
			'When authentication is provided, a POST must return an OK response and the data selection must have been created'
		)
	...
	def test_get_detail(self):
		...
		response = self.api_client.get(
			self.get_resource_uri(self.test_data_selection), format='json', authentication=self.other_user_authentication
		)
		self.assertHttpUnauthorized(response, msg=msg)
	...
	def test_get_detail_metadata(self):
		...
		msg = (
			'When no query_string is specified, the metadata field resource_uri must be exactly the metadata ressource list url'
		)
		response = self.api_client.get(
			self.get_resource_uri(self.test_data_selection), format='json', authentication=self.test_user_authentication
		)
		self.assertGetDetailResponseContains(
			response,
			metadata={'resource_uri': self.get_resource_uri(resource_name='base_metadata_test_resource'), 'count': 2},
			msg=msg,
		)
		...
		self.assertGetDetailResponseContains(
			response,
			metadata={
				'resource_uri': self.get_resource_uri(resource_name='base_metadata_test_resource') + '?oid=test_metadata1',
				'count': 1,
			},
			msg=msg,
		)
	...
	def test_patch_detail(self):
		...
		response = self.api_client.patch(
			self.get_resource_uri(self.test_data_selection),
			data=data,
			format='json',
			authentication=self.other_user_authentication,
		)
		self.assertHttpUnauthorized(response, msg=msg)
