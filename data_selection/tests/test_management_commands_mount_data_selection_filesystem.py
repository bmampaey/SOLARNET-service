...
class TestDataSelectionFilesystemOperations(StaticLiveServerTestCase):
	...
	def setUp(self):
		...
		# Create an instance of the Opertions class
...
class TestMountDataSelectionFilesystemManagementCommand(StaticLiveServerTestCase):
	...
	def setUp(self):
		...
		process = subprocess.run([settings.BASE_DIR / 'manage.py', 'mount_data_selection_filesystem', self.mount_dir.name, '--user', self.user_name, '-l', self.log_file.name , '--verbosity', '2', '--settings', 'project.settings.test'], timeout = 5 * 60, capture_output = True)
