...
class TestDataSelectionFilesystemOperations(StaticLiveServerTestCase):
	...
	def setUpClass(cls):
		...
		for file_path in ['static/test_file1.fits', 'static/test_file2.fits']:
			...
			cls.test_files.append(
				{'url': urljoin(cls.live_server_url, file_path), 'content': file_content, 'size': len(file_content)}
			)

	def setUp(self):
		# Create test data
		self.test_user = User.objects.create_user(
			username='test@test.com', email='test@test.com', password='test', first_name='test', last_name='test'
		)
		self.test_dataset = create_test_dataset(name='test dataset')
		self.test_data_location1 = self.test_dataset.data_locations.create(
			file_url=self.test_files[0]['url'], file_size=self.test_files[0]['size'], file_path='test_path/file1.fits'
		)
		self.test_data_location2 = self.test_dataset.data_locations.create(
			file_url=self.test_files[1]['url'],
			file_size=self.test_files[1]['size'],
			file_path='test_path/file2.fits',
			offline=True,
		)
		self.test_metadata1 = self.test_dataset.metadata_model.objects.create(
			oid='test_metadata1', data_location=self.test_data_location1
		)
		self.test_metadata2 = self.test_dataset.metadata_model.objects.create(
			oid='test_metadata2', data_location=self.test_data_location2
		)
		self.test_data_selection = self.test_user.data_selections.create(
			dataset=self.test_dataset, description='test data selection'
		)
	...
	def test_get_data_location_values(self):
		...
		self.assertQuerysetEqual(
			...
			[
				{
					'file_url': self.test_data_location1.file_url,
					'file_size': self.test_data_location1.file_size,
					'file_path': self.test_data_location1.file_path,
					'thumbnail_url': self.test_data_location1.thumbnail_url,
					'update_time': self.test_data_location1.update_time,
				}
			],
		)
		...
		data_location_values = ['file_path']
		extra_values = {
			'sub_path': Func(F('file_path'), Value('/'), Value(1), function='SPLIT_PART', output_field=CharField())
		}
		result = self.operations.get_data_location_values(self.test_data_selection, *data_location_values, **extra_values)
		self.assertQuerysetEqual(
			result,
			[{'file_path': self.test_data_location1.file_path, 'sub_path': self.test_data_location1.file_path.split('/')[0]}],
		)
	...
	def test_get_data(self):
		...
		offset, length = 0, 10
		self.assertEqual(
			self.operations.get_data(self.test_files[0]['url'], offset, length),
			self.test_files[0]['content'][offset : offset + length],
			msg=msg,
		)
	...
	def test_getattr(self):
		...
		msg = (
			'When the path is /uuid where the UUID corresponds to a data selection, getattr must return a stat dict for a directory'
			''
		)
		path = '/%s' % self.test_data_selection.uuid
		result = self.operations.getattr(path)
	...
	def test_readdir(self):
		...
		msg = (
			'When the path is /uuid where the UUID corresponds to a data selection, readdir must return a list of sub directories of the online data location'
			''
		)
		path = '/%s' % self.test_data_selection.uuid
		result = self.operations.readdir(path)
		...
		try:
			process = subprocess.run(
				[
					...
					'mount_data_selection_filesystem',
					self.mount_dir.name,
					'--user',
					self.user_name,
					'-l',
