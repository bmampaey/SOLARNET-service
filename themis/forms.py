...
from themis.models import Metada, Tag
...
class SearchData(BaseForm):
	...
	THEMIS_WAVELENGTHS = Metada.objects.values_list('wavelnth', flat=True).distinct()
	...
	wavelengths = forms.TypedMultipleChoiceField(required=False, coerce=int, widget=forms.SelectMultiple(), initial = THEMIS_WAVELENGTHS, choices=[(w, u'%sÅ'%w) for w in THEMIS_WAVELENGTHS])
